...
pub fn gpu_lagrange_interpolate(
    ...
    let n_padded = ((n_points + 31) / 32) * 32;
    let num_workspaces = n_padded / 32;
    let mut workspaces = unsafe { DeviceBuffer::<Fe>::zeroed(num_workspaces * n_padded)? };
    let lagrange_kernel = ctx.module.get_function(CStr::from_bytes_with_nul(b"lagrange_contribution_systolic\0")?)?;
    
    let contrib_grid = (n_padded as u32 + block_size - 1) / block_size;
